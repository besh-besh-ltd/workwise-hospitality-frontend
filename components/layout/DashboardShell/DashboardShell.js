...
const DashboardShell = ({ children }) => {
  ...
  const handleLogoutConfirm = useCallback(async () => {
    ...
    // Clearing browser storage is not signing out: the session cookie is
    // httpOnly and lives on the server. Without this call it survives, and the
    // login page's getServerSideProps sees a valid session and bounces straight
    // back to /dashboard/buyer — which is exactly what logout appeared to do.
    try {
      await fetch("/api/auth/logout", { method: "POST" });
    } catch (_) {
      /* fall through — still leave the dashboard */
    }
    // Full navigation, not router.push: the login page is server-rendered and
    // has to be fetched fresh so it reads the now-cleared cookie.
