...
export default function UpdateGRNModal({ show, onClose, onAction }) {
  ...
  const handleRaiseInvoice = async () => {
    ...
    try {
      ...
      toast.warning("Something went wrong while updating GRN, please try again!")
    } finally {
      setFileData(prev => ({...prev, loading: false}))
    }
  };

  const fileError = touched && !fileData.file ? "Please select an GRN file." : "";
  ...
  return (
    ...
    >
      <Modal.Header closeButton className="px-3 pt-3">
        <Modal.Title className="fw-semibold fs-5">Attach GRN Document</Modal.Title>
      </Modal.Header>

      <Modal.Body className="pt-2">
        <Form.Group controlId="invoiceFile">
          <Form.Label className="fw-semibold mb-1">Goods Receipt Note</Form.Label>
          <p className="text-muted mb-3 text-sm" style={{ lineHeight: 1.5 }}>
            Upload the GRN document you want to attach to this Purchase Order.
            This document will be stored with this PO, and everyone associated with it
          ...
          <div className="d-flex flex-column gap-2">
            ...
            <div className="form-text text-dark mt-1">
              <span className="fw-semibold">Important:</span> Once the file is
      ...
      <Modal.Footer className="border-0">
        ...
        <Button
          ...
          className="p-2 px-3"
        >
          {fileData.loading ? 'Loading...' : 'Update GRN'}
        </Button>
      </Modal.Footer>
