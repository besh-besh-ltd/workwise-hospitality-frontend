...
const EditRFQSchema = Yup.object().shape({
  contact_number: Yup.string()
    .matches(
      /^\d+$/,
      "Please enter only numbers without country code or special characters"
    )
...
const EditRFQ = () => {
  ...
  const fetchAvailableVendorsForProduct = async () => {
    ...
    try {
      const body = {
        ...
        excludeIds:
          selectedProduct?.vendors?.map((vendor) => vendor.user_id) ?? [],
      };
  ...
  useEffect(() => {
    ...
    if (
      ...
      initialDataLoaded &&
      !termRefreshCompletedRef.current
    ) {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (
        rfqData.terms &&
        rfqData.terms.length > 0 &&
        availableTerms.length > 0
      ) {
        // Create a map of available terms for quick lookup
        const termMap = new Map(
          availableTerms.map((term) => [String(term.id), term])
        );
  ...
  const handleUpdateRFQ = async (formValues) => {
    try {
      ...
      if (
        formValues.project_id != rfqData.project_id &&
        (!isNaN(formValues.project_id) || formValues.project_id == null)
      ) {
      ...
      if (
        formValues.rfq_type &&
        rfqTypes.some((type) => formValues.rfq_type == type.value)
      ) {
      ...
      if (
        dataToSend.reverse_auction &&
        (!formValues.ra_start_date || !formValues.ra_end_date)
      ) {
        console.log(
          "REVERSE AUCTION: ",
          dataToSend.reverse_auction,
          " RE_START_DATE: ",
          formValues.ra_start_date,
          " RE_END_DATE: ",
          formValues.ra_end_date
        );
     ...
     return;
      ...
      updateRfq(dataToSend)
        .then((response) => {
          ...
          if (isSuccess) {
            toast.info(
              "RFQ has been updated, you can change something else or go back!"
            );
        ...
        .catch((error) => {
          ...
          if (
            ...
            toast.error(
              "Failed to update RFQ. Please check form fields and try again."
            );
          }
        })
        .finally(() =>
      ...
      toast.error(
        "An error occurred while updating the RFQ: " +
  ...
  const renderDeletedProductsTable = () => {
    if (
      !updatableData.products?.deletable ||
      updatableData.products?.deletable.length === 0
    ) {
      return;
    }

    const handleQuantityProcessing = (product) => {
                  ...
                  .map((product) => {
                    return (
                      <tr key={product.product_details?.[0]?.id}>
                        ...
                        <td>{handleQuantityProcessing(product)}</td>
                        <td>
    ...
    return (
      <div className="container-fluid">
        <div className="alert alert-danger">
          <p>
            <strong>Error loading RFQ data:</strong> {dataFetchError}
          </p>
          <button
  ...
  return (
    <>
                    ...
                    return (
                      <Item
                        ...
                        onFilesChange={(change) =>
                          handleFileChange(product, change)
                        }
                        ...
                        handleViewVendorInEdit={() =>
                          handleShowVendorModal(product, setShowVendorModal)
                        }
                        handleAddVendorInEdit={() =>
                          handleShowVendorModal(product, setShowAddVendorModal)
                        }
              ...
              <Form onSubmit={handleSubmit}>
                    ...
                    <div className="row g-3">
                      ...
                      <div className="col-md-6">
                        ...
                        <div className="mb-3">
                          ...
                          <Select
                            key={`project-select-${
                              rfqFormDataFromStore.project_id || "none"
                            }`}
                            options={projects}
                            value={(() => {
                              if (!rfqFormDataFromStore.project_id) return null;
                              const projectId = parseInt(
                                rfqFormDataFromStore.project_id
                              );
                              const match = projects.find(
                                (p) => parseInt(p.value) === projectId
                              );
                              return match || null;
                            })()}
                            onChange={(selectedOption) => {
                              const projectId = selectedOption
                                ...
                                : null;
                              dispatch(
                                setOtherFormFields({ project_id: projectId })
                              );
                              setHasUnsavedChanges(true);
                            }}
                        ...
                        <div className="mb-3">
                          ...
                          <Select
                            ...
                            value={(() => {
                              ...
                              const match = rfqTypes.find(
                                (p) => p.value === rfqType
                              );
                              return match || null;
                            })()}
                            onChange={(selectedOption) => {
                              const rfqType = selectedOption
                                ? selectedOption.value
                                : null;
                              dispatch(
                                setOtherFormFields({ rfq_type: rfqType })
                              );
                              setHasUnsavedChanges(true);
                            }}
                      ...
                      <div className="col-md-6">
                        ...
                        <div className="mb-3">
                          ...
                          <Select
                            options={binaryType}
                            value={(() => {
                              const reverseAuction = parseInt(
                                rfqFormDataFromStore.reverse_auction
                              );
                              const match = binaryType.find(
                                (p) => p.value == reverseAuction
                              );
                              return match ?? null;
                            })()}
                            onChange={(selectedOption) => {
                              const reverseAuction = selectedOption
                                ? parseInt(selectedOption.value)
                                : null;
                              dispatch(
                                setOtherFormFields({
                                  reverse_auction: reverseAuction,
                                })
                              );
                              setHasUnsavedChanges(true);
                            }}
                      ...
                      {!!rfqFormDataFromStore.reverse_auction &&
                        parseInt(rfqFormDataFromStore.reverse_auction) && (
                          <>
                            <div className="col-md-6">
                              <div className="mb-3">
                                <label className="form-label fw-medium">
                ...
                <div className="card mb-4">
                  ...
                  <div className="card-body">
                      ...
                      <div className="terms-list border rounded p-3 bg-light">
                        {selectedTerms && selectedTerms.length > 0 ? (
                          ...
                          <p className="text-muted mb-0">
                            No terms have been selected for this RFQ.
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="border rounded p-3 bg-light">
                        {rfqData.comment && rfqData.comment.trim() ? (
                          <div className="mb-0">{rfqData.comment}</div>
                        ) : (
                          <p className="text-muted mb-0">
                    ...
                    {rfqData?.term_and_condition_files &&
                      rfqData.term_and_condition_files.length > 0 && (
                        <div className="mt-4">
                          <h6 className="mb-3 fw-medium">
                            Terms & Conditions Files{" "}
                          </h6>
                          <div className="row g-2">
                            {rfqData.term_and_condition_files.map(
                              (file, idx) => (
                  ...
                  <div className="d-flex flex-column">
                    ...
                    {isUpdateConfirm && (
                      <span className="text-danger mt-2">
                        Updation have deletable products,
                        <br />
      ...
      <AddVendorModal
        headerTitle={`Add Vendors for ${
          selectedProduct?.product?.variant_name ?? "-"
        }`}
        vendors={vendors}
        productData={selectedProduct}
        updatableData={updatableData}
