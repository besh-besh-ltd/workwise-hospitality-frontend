...
import { StageNoPermission, removalReasonLabel } from "./StageShared";
              ...
              {approval ? (
                <>
                  ...
                  {steps.map((step) => {
                    ...
                    // REMOVED approver rows are a mid-flight reconciler's soft-
                    // tombstone (role revoked while the approval was in progress)
                    // — exclude them from the live list, show them separately
                    // and muted instead.
                      ...
                      <div key={step.step_order}>
                        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", margin: "10px 0 2px" }}>
                          ...
                          <span className={`status-pill ${step.status === "APPROVED" ? "success" : step.status === "REJECTED" ? "danger" : isStepSkipped || isStepRemoved ? "neutral" : isCurrent ? "warn" : "neutral"}`} style={{ fontSize: 9.5 }}>
                            <span className="dot" />
                            {step.status === "APPROVED" ? "cleared" : step.status === "REJECTED" ? "sent back" : isStepSkipped ? "skipped" : isStepRemoved ? "removed" : isCurrent ? "reviewing now" : "waiting"}
                          </span>
                        </div>
