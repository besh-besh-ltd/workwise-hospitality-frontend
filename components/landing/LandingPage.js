...
const LandingPage = () => {
  ...
  // The demo has no real accounts — "Log in" goes to the persona picker
  // instead of the production auth drawer (which needs OAuth, OTP and a
  // payment flow this build deliberately does not carry).
  const openAuth = () => { window.location.href = '/login'; };
  const openRegister = () => { window.location.href = '/login'; };
