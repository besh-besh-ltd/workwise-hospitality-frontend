...
// Items #270+ (amendment, MR specifics) live on their own pages.
...
// ──────────────────────────────────────────────────────────────────────────
//  Tiny inline icons (kept inline so the page has no extra import cost)
// ──────────────────────────────────────────────────────────────────────────
...
export default function ActiveContractPage() {
  ...
  if (!arc) {
    return <main className="main-body"><div className="empty-state"><h2>Contract not found</h2><p>Check the URL.</p></div></main>;
  }

  const daysLeft = daysBetween(arc.contract_end_at);
  const statusActive = arc.status === "contract_active";

  return (
    <main className="main-body" style={{ paddingBottom: 32 }}>
      {/* Breadcrumbs — same pattern as the PO module */}
      <nav className="arc-crumb-strip" aria-label="Breadcrumb">
        <div className="inner">
          <button type="button" className="crumb-back" onClick={() => router.push("/dashboard/buyer/rate-contracts/all")}>
            <Icon size={13}><line x1="19" y1="12" x2="5" y2="12" /><polyline points="12 19 5 12 12 5" /></Icon>
            Back
          </button>
          <span className="sep">/</span>
          <Link href="/dashboard/buyer/rate-contracts">Rate contracts</Link>
          <span className="sep">/</span>
          <Link href="/dashboard/buyer/rate-contracts/all">All contracts</Link>
          <span className="sep">/</span>
          <span className="here">#{arc.arc_number}</span>
        </div>
      </nav>
        ...
        <div className="top">
          <div>
            ...
            <h1>
              ...
              <span className={`status-chip ${statusActive ? "active" : (daysLeft <= 60 ? "expiring" : "active")}`}>{statusActive ? (daysLeft <= 60 ? "Expiring soon" : "Active") : arc.status}</span>
            </h1>
            <div className="sub">
              <span className="kv"><span className="kv-k">Category</span><span className="kv-v em">{arc.category_title || "—"}</span></span>
              <span className="kv"><span className="kv-k">Department</span><span className="kv-v">{arc.department_title || "—"}</span></span>
              <span className="kv"><span className="kv-k">Eligibility</span><span className="kv-v">{arc.eligibility_type === "open" ? "Open" : "Invitation"}</span></span>
            </div>
          </div>
          <div className="hero-actions">
            <button className="btn btn-sm"><I.callOff /> Release call-off PO</button>
            <button className="btn btn-sm"><I.renew /> Renew</button>
          </div>
        </div>
        <div className="hero-detail-grid">
          <div className="cell"><div className="k">Term</div><div className="v"><span className="em mono">{fmtDate(arc.contract_start_at)}</span> → <span className="mono">{fmtDate(arc.contract_end_at)}</span></div></div>
          <div className="cell"><div className="k">Days remaining</div><div className="v"><span className="em mono">{daysLeft}</span> days</div></div>
          <div className="cell"><div className="k">Business unit</div><div className="v"><span className="em">{arc.hotel_name || "—"}</span>{arc.hotel_city ? <> · <span>{arc.hotel_city}</span></> : null}</div></div>
          <div className="cell"><div className="k">Projected value</div><div className="v"><span className="em mono">{fmtL(withTaxes ? kpi.projectedTotal : kpi.projected)}</span> <span style={{ color: "rgba(255,255,255,0.6)", fontSize: 11 }}>{withTaxes ? "(incl. tax)" : "(excl. tax)"}</span></div></div>
          <div className="cell"><div className="k">Consumed YTD</div><div className="v"><span className="em mono">{fmtL(withTaxes ? kpi.consumedTotal : kpi.consumed)}</span> · <span>{kpi.drawnPct}%</span></div></div>
        </div>
      </section>
      ...
      <section className="stat-strip" style={{ gridTemplateColumns: "repeat(5,1fr)" }}>
        ...
        <div className="stat-card">
          ...
          <div><div className="s-val mono">{daysLeft}</div><div className="s-label">Days remaining</div></div>
        </div>
      </section>
                                ...
                                <div className="cvh-row">
                                  ...
                                  <div className="cvh-main">
                                    <div className="cvh-name">
                                      ...
                                      <span className="cvh-active"><I.check /> Active</span>
                                    </div>
                                      ...
                                      <td key={v.vendorId} className="cons-cell">
                                        <div className={`cons-content ${ln.pct >= 100 ? "danger" : ln.pct >= 75 ? "warn" : "ok"}`}>
                                          ...
                                          {/* Both qty and ₹ value on each row. Value = qty × (B.P + Frt)
                                              (incl. GST when the hero "incl. tax" toggle is on). */}
              ...
              <div className="section-body flush">
                {callOffs.length === 0 ? (
                  <div className="empty-state">
                    ...
                    <p>Release the first call-off against this contract using the button above.</p>
                  </div>
                ) : (
                  <>
            ...
            <div className="section-body" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {byVendor.map((b) => (
                <div key={b.vendorId} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <span className="balance-pill on-track" style={{ flexShrink: 0 }}><I.check /> Active</span>
                </div>
              ))}
            </div>
          </div>
...
// ──────────────────────────────────────────────────────────────────────────
//  Legacy AmendmentModal — kept for the future vendor portal page. Not
//  wired into the buyer's active page (vendor-initiated only per stakeholder
//  direction). When the vendor portal lands, this is the modal it opens.
// ──────────────────────────────────────────────────────────────────────────
const TODAY_ISO = () => new Date().toISOString().slice(0, 10);

function AmendmentModal(props) {
  // Portal-render to document.body so any ancestor stacking context (the
  // DashboardShell uses z-index up to ~1080) can't trap the backdrop
  // behind the sidebar/topbar.
  const [mounted, setMounted] = useState(false);
  useEffect(() => { setMounted(true); }, []);
  if (!mounted || typeof document === "undefined") return null;
  return createPortal(<AmendmentModalInner {...props} />, document.body);
}

function AmendmentModalInner({ arc, contracts, allLines, busy, error, onClose, onSubmit }) {
  const [type, setType]             = useState("price");
  const [contractId, setContractId] = useState(contracts[0]?.contract?.id || "");
  const [lineId, setLineId]         = useState("");           // arc_contract_line_id
  const [newRate, setNewRate]       = useState("");
  const [newQty, setNewQty]         = useState("");
  const [newItemVariantId, setNewItemVariantId] = useState(""); // item_add
  const [from, setFrom]             = useState(TODAY_ISO());
  const [to, setTo]                 = useState("");
  const [reason, setReason]         = useState("");

  // Lines belonging to the selected contract — drives item dropdowns.
  const linesForContract = useMemo(
    () => allLines.filter((l) => String(l.contractId) === String(contractId)),
    [allLines, contractId]
  );
  const lineSelected = linesForContract.find((l) => String(l.arc_contract_line_id) === String(lineId));

  function valid() {
    if (!contractId || !reason.trim() || !from) return false;
    if (type !== "term" && type !== "item_remove" && !to) return false;
    if (type === "price")       return !!lineId && Number(newRate) > 0;
    if (type === "qty")         return !!lineId && Number(newQty)  > 0;
    if (type === "item_add")    return !!newItemVariantId && Number(newQty) > 0 && Number(newRate) > 0;
    if (type === "item_remove") return !!lineId;
    if (type === "term")        return !!from;
    return false;
  }
  ...
  function build() {
    const base = {
      ...
      amendment_type:  type,
      amendment_from:  from,
      amendment_to:    (type === "term" || type === "item_remove") ? null : to,
      reason: reason.trim(),
      payload: {},
    };
    if (type === "price")       base.payload = { arc_contract_line_id: Number(lineId), new_rate: Number(newRate) };
    if (type === "qty")         base.payload = { arc_contract_line_id: Number(lineId), new_qty: Number(newQty) };
    if (type === "item_add")    base.payload = { product_variant_id: Number(newItemVariantId), new_qty: Number(newQty), new_rate: Number(newRate) };
    if (type === "item_remove") base.payload = { arc_contract_line_id: Number(lineId) };
    return base;
  }

  const pctChange = (() => {
    if (type !== "price" || !lineSelected || !Number(newRate)) return null;
    const cur = Number(lineSelected.rate);
    if (!cur) return null;
    return ((Number(newRate) - cur) / cur) * 100;
  })();
  ...
  return (
    <div className="arc-modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="arc-modal lg">
        <div className="modal-head">
          <div>
            <div className="t"><h3>Create amendment</h3></div>
            <div className="sub">Routes through committee review · Effective-from + to are mandatory</div>
          </div>
          <button className="icon-btn" onClick={onClose} aria-label="Close">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6L6 18M6 6l12 12"/></svg>
          </button>
        ...
        <div className="modal-body">
          ...
          {/* Type radios */}
          <label className="label">Amendment type <span className="req">*</span></label>
          <div className="form-grid cols-3" style={{ marginTop: 6 }}>
            {[
              { v: "price",       l: "Price revision" },
              { v: "qty",         l: "Quantity revision" },
              { v: "item_add",    l: "Item add" },
              { v: "item_remove", l: "Item remove" },
              { v: "term",        l: "Term extension" },
            ].map((opt) => (
              <label key={opt.v} className="rd">
                <input type="radio" name="atype" value={opt.v} checked={type === opt.v} onChange={() => { setType(opt.v); setLineId(""); setNewRate(""); setNewQty(""); setNewItemVariantId(""); }} />
                <span className="rd-box" />{opt.l}
              </label>
            ))}
          </div>
          <div className="help-text" style={{ marginTop: 6 }}>
            BU change isn't supported — every ARC is single-BU at creation.
          </div>

          {/* Contract picker (split-award contracts produce N rows) */}
          {contracts.length > 1 && (
            <div style={{ marginTop: 18 }}>
              <label className="label">Vendor contract <span className="req">*</span></label>
              <select className="select" value={contractId} onChange={(e) => { setContractId(e.target.value); setLineId(""); }}>
          ...
          <div style={{ marginTop: 18, paddingTop: 18, borderTop: "1px solid var(--border)" }}>
            {(type === "price" || type === "qty" || type === "item_remove") && (
              <div className="form-grid" style={{ marginBottom: 14 }}>
                <div>
                  ...
                  <select className="select" value={lineId} onChange={(e) => setLineId(e.target.value)}>
                    ...
                    {linesForContract.map((l) => (
                      <option key={l.arc_contract_line_id} value={l.arc_contract_line_id}>
                        {l.variant_name} ({l.variant_slug || l.arc_item_id})
                      </option>
                    ))}
                  </select>
                </div>
                {lineSelected && type === "price" && (
                  <div>
                    <label className="label">Current rate</label>
                    <div className="input-group">
                      <div className="prefix">₹</div>
                      <input className="input mono" value={lineSelected.rate} readOnly />
                      <div className="suffix">/{lineSelected.uom || "unit"}</div>
                    </div>
                  </div>
                )}
                {lineSelected && type === "qty" && (
                  <div>
                    <label className="label">Current committed quantity</label>
                    <div className="input-group">
                      <input className="input mono" value={lineSelected.committed} readOnly />
                      <div className="suffix">{lineSelected.uom || "unit"}</div>
                    </div>
                  </div>
                )}
            ...
            {type === "price" && lineSelected && (
              <div>
                <label className="label">New revised rate <span className="req">*</span></label>
                <div className="input-group" style={{ maxWidth: 280 }}>
                  <div className="prefix">₹</div>
                  <input type="number" className="input input-num" value={newRate} onChange={(e) => setNewRate(e.target.value)} placeholder="0.00" min={0} step="0.01" />
                  <div className="suffix">/{lineSelected.uom || "unit"}</div>
                </div>
                {pctChange != null && (
                  <div className="help-text" style={{ marginTop: 6 }}>
                    Change: <span className="mono">{pctChange.toFixed(1)}%</span>
                    <span style={{ color: pctChange > 0 ? "var(--danger)" : "var(--success)", marginLeft: 6, fontWeight: 600 }}>
                      ({pctChange > 0 ? "increase" : "decrease"})
                    </span>
                  </div>
            ...
            {type === "qty" && lineSelected && (
              <div>
                <label className="label">New committed quantity <span className="req">*</span></label>
                <div className="input-group" style={{ maxWidth: 280 }}>
                  <input type="number" className="input input-num" value={newQty} onChange={(e) => setNewQty(e.target.value)} placeholder="0" min={0} />
                  <div className="suffix">{lineSelected.uom || "unit"}</div>
                </div>
              </div>
            )}
            ...
            {type === "item_add" && (
              <div>
                <label className="label">Product variant ID <span className="req">*</span></label>
                <input className="input mono" value={newItemVariantId} onChange={(e) => setNewItemVariantId(e.target.value)} placeholder="e.g. 1234" style={{ maxWidth: 280 }} />
                <div className="help-text" style={{ marginTop: 4 }}>Pick a variant from the same category. A catalogue picker lands with Phase C.</div>
                <div className="form-grid" style={{ marginTop: 14 }}>
                  <div>
                    <label className="label">Committed quantity <span className="req">*</span></label>
                    <div className="input-group">
                      <input type="number" className="input input-num" value={newQty} onChange={(e) => setNewQty(e.target.value)} placeholder="0" min={0} />
                      <div className="suffix">units</div>
                    </div>
                  </div>
                  <div>
                    <label className="label">Rate <span className="req">*</span></label>
                    <div className="input-group">
                      <div className="prefix">₹</div>
                      <input type="number" className="input input-num" value={newRate} onChange={(e) => setNewRate(e.target.value)} placeholder="0.00" min={0} step="0.01" />
                    </div>
            ...
            {type === "item_remove" && lineSelected && (
              <div className="help-text" style={{ color: "var(--danger)" }}>
                Removing this item closes its committed quantity. In-flight call-off POs remain valid; new call-offs against this item won't be allowed.
              </div>
            )}
            ...
            {type === "term" && (
              <div>
                ...
                <div className="input-group" style={{ maxWidth: 280 }}>
                  <input className="input mono" value={fmtDate(arc.contract_end_at)} readOnly />
                </div>
                <label className="label" style={{ marginTop: 14 }}>New end date <span className="req">*</span></label>
                <input type="date" className="input" value={from} onChange={(e) => setFrom(e.target.value)} style={{ maxWidth: 280 }} />
              </div>
            )}
          </div>

          {/* Effective window + reason — common to every type */}
          {type !== "term" && (
            <div style={{ marginTop: 18, paddingTop: 18, borderTop: "1px solid var(--border)" }}>
          ...
          <label className="label" style={{ marginTop: 14 }}>Reason / justification <span className="req">*</span></label>
          <textarea className="textarea" value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Explain why this amendment is required (commercial, operational, regulatory…)" />

          {error && (
            <div className="guide" style={{ marginTop: 12, background: "var(--danger-soft)", borderColor: "rgba(185,28,28,0.2)" }}>
              <div className="g-ic" style={{ color: "var(--danger)" }}><I.info /></div>
              <div style={{ color: "var(--danger)" }}>{error}</div>
            </div>
          )}
        </div>

        <div className="modal-foot" style={{ padding: "14px 22px", borderTop: "1px solid var(--border)", display: "flex", justifyContent: "flex-end", gap: 8 }}>
          <button className="btn btn-ghost" onClick={onClose} disabled={busy}>Cancel</button>
          <button className="btn btn-blue" disabled={!valid() || busy} onClick={() => onSubmit(build())}>
      ...
      <section className="arc-sk-hero">
        <div style={{ display: "flex", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
          <div style={{ minWidth: 0, flex: 1 }}>
            ...
            <Sk w="62%" h={26} style={{ marginBottom: 12 }} />
        ...
        <div style={{ marginTop: 20, paddingTop: 18, borderTop: "1px solid rgba(255,255,255,0.10)", display: "grid", gridTemplateColumns: "repeat(5,1fr)", columnGap: 28, rowGap: 18 }}>
