...
export function buildVendorMatchPlan(state) {
  ...
  const _build = function () {
      ...
      return {
        ...
        // The prototype navigated to its own vendors.html; that wiring was
        // dropped on the port and never replaced, so the button did nothing.
        // It now carries the shortlist onto the draft, where the wizard's
        // vendor step reads it back.
        onDone: function (body) {
          ...
          btn.addEventListener('click', function () {
            ...
            var picked = (b.recommended || []).map(function (v) {
              return { id: v.id, name: v.name, score: v.score };
            });

            try {
              window.sessionStorage.setItem(
                'ihg:recommendedVendors',
                JSON.stringify({ at: new Date().toISOString(), vendors: picked })
              );
            } catch (e) { /* private mode — the confirmation still stands */ }

            setTimeout(function () {
              btn.dataset.done = '1';
              btn.innerHTML = '<span>Applied \u2014 ' + picked.length +
                ' vendor' + (picked.length === 1 ? '' : 's') + ' carried to the draft</span>';
            }, 600);
          });
        }
      };
...
export const stagedFromNames = (names = []) => {
  // No selection means no match. The fallback below is for names that were
  // given but did not resolve to a catalogue entry — using it for an EMPTY
  // basket is what let the panel rank vendors against nothing at all.
