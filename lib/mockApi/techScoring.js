...
/**
 * One vendor's standing on one product, from the marks actually given.
 *
 * The vendor header chip renders `calculated_score` / `is_passed` — leaving
 * them at 0/false meant a vendor scored 89 out of 100 still displayed
 * "0% FAIL" beside its own passing marks.
 */
export const vendorStanding = (clauses, vendorId, minScore) => {
  const max = clauses.reduce((s, c) => s + (c.weightage || 0), 0);
  let scored = 0;
  let total = 0;
  clauses.forEach((c) => {
    const mark = markFor(c.clause_id, vendorId);
    if (!mark || mark.marks == null) return;
    scored += 1;
    total += Number(mark.marks) || 0;
  });

  const complete = clauses.length > 0 && scored === clauses.length;
  const pct = max ? Math.round((total / max) * 100) : 0;
  return {
    has_marks: scored > 0,
    calculated_score: total,
    percentage: complete ? pct : 0,
    // Only a fully scored vendor carries a verdict — a part-marked one that
    // happens to clear the bar has not been evaluated yet.
    is_passed: complete && pct >= minScore,
    is_failed: complete && pct < minScore,
    is_cleared: complete && pct >= minScore,
  };
};
