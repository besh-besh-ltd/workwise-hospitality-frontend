...
import { vendorStanding } from "./techScoring";
import { itRfq, itLineItems, itVendors, itVendorsById, itRate, itClauses, IT_CATEGORY, IT_MIN_SCORE } from "@/data/ihg/itRfq";
...
/** IT vendors sit in their own numeric range so they cannot collide with the linen base. */
export const itVendorNumericId = (slug) => 7301 + itVendors.findIndex((v) => v.id === slug);

/**
 * The IT RFQ's comparison grid.
 *
 * Built separately from the linen fixture because its suppliers, items and
 * charge behaviour are its own — and because `locked` here follows the
 * TECHNICAL evaluation, not the bid deadline.
 */
export const itCompareView = (id, withFreight = true, technicalDone = false) => {
  // Which vendors cleared the technical gate, per product. Once the
  // evaluation is signed off only those carry a comparable price. A vendor
  // held out on technical DID quote, so it is marked disqualified rather than
  // silently dropped — dropping it reads as an unresponsive supplier.
  const groups = techEvalGroupsFor(id);
  const standingsFor = (productId) => {
    if (!technicalDone) return null;
    const group = groups.find((g) => g.rfq_product_id === productId);
    if (!group) return null;
    const map = {};
    group.vendors.forEach((v) => {
      map[v.vendor_id] = vendorStanding(group.clauses, v.vendor_id, IT_MIN_SCORE);
    });
    return map;
  };

  const products = itLineItems.map((item, idx) => {
    const cells = {};
    const absence = {};
    const standings = standingsFor(idx + 1);
    itVendors.forEach((v, vIdx) => {
      const standing = standings?.[itVendorNumericId(v.id)];
      if (standing && standing.is_failed) {
        absence[String(itVendorNumericId(v.id))] = {
          status: "TECH_FAILED",
          tech_score: standing.percentage,
          min_score: IT_MIN_SCORE,
        };
        return;
      }
      const rate = itRate(v.id, item.id);
      const subtotal = rate * item.annualQty;
      // Freight on hardware is a flat per-unit install charge rather than a
      // percentage — it is what the buyer actually negotiates on IT kit.
      const install = Math.round(item.annualQty * (v.factor > 1 ? 900 : 650));
      const taxAmt = Math.round(((subtotal + install) * GST_PCT) / 100);
      const charges = withFreight
        ? [{ name: "Installation", slug: "installation", amount: install, amount_mode: "amount", is_global: false, tax: GST_PCT, tax_mode: "percentage" }]
        : [];
      const chargeSum = withFreight ? install : 0;

      cells[itVendorNumericId(v.id)] = {
        base: rate,
        subtotal,
        freight: 0,
        packaging: 0,
        ...
        tax_amt: taxAmt,
        total: subtotal + chargeSum + taxAmt,
        delivery: `${v.leadDays} days`,
        pay: "30 days from delivery & sign-off",
        comment: `Quoted against the RFQ specification. On-site SLA per clause 3. ${v.city} stock.`,
        docs: [],
        missing: false,
        history: [],
        finalize: {
          ...
          quote_id: 8800 + vIdx,
          quote_item_id: (idx + 1) * 1000 + vIdx,
          unit_price: rate,
          total_value: subtotal + chargeSum + taxAmt,
          charges_meta: charges,
        },
      };
    });
    ...
    return {
      ...
      awaiting_me: false,
      reject_info: null,
      quoted_count: itVendors.length,
      lpr: { rate: item.lastRate, landed_unit: Math.round(item.lastRate * (1 + GST_PCT / 100)) },
      // Clauses ARE configured here — that is the whole point of this RFQ.
      tech: { configured: true, scores: {} },
      quotes_absence: absence,
      quotes: cells,
    };
  });

  const vendors = itVendors.map((v, i) => ({
    id: itVendorNumericId(v.id),
    name: v.name,
    short: v.short,
    initials: v.short.slice(0, 2).toUpperCase(),
    quote_pct: 100,
    tech_score: null,
    track_record: `${v.onTimePct}% on time`,
    orders_done: 2 + i,
    pos_accepted: 2 + i,
    po_value: 4200000 + i * 900000,
    is_new: false,
    invited_rfqs: 1,
    quoted_rfqs: 1,
  }));

  const owner = peopleById[itRfq.ownerId] || Object.values(peopleById)[0];

  return {
    rfq: {
      id: Number(id) || Number(itRfq.id),
      rfq_no: String(id),
      number: String(id),
      title: itRfq.title,
      company: company.name,
      hotel: itRfq.propertyIds.map((p) => propertiesById[p]?.shortName).join(", "),
      department: itRfq.department,
      contact: owner.name,
      phone: owner.phone,
      email: company.procurementEmail,
      delivery_location: propertiesById["ic-mumbai"].address,
      deadline: `${itRfq.closesOn}T18:00:00+05:30`,
      status: "Quotes received",
      reverse_auction: false,
      tech_clauses: itClauses.length,
      quotes_invited: vendors.length,
      quotes_received: vendors.length,
      rounds: { ended: 0, active: 0 },
      project_id: -1,
    },
    bid_end_date: `${itRfq.closesOn}T18:00:00+05:30`,
    // The rule, made real: sealed until the technical evaluation is signed off.
    quotes_locked: !technicalDone,
    quotes_locked_reason: "technical",
    categories: [{ id: 2, title: IT_CATEGORY }],
    has_delivery_charges: true,
    approval_chain: [],
    stage_actors: [],
    vendors,
