...
import { markFor, scoredResponse, evaluationResult } from "./techScoring";
...
const itTechEvalGroups = () => {
  const vendors = itVendors.map((v, idx) => ({
    vendor_id: itVendorNumericId(v.id),
    rfq_product_vendor_id: 950 + idx,
    vendor_name: v.name,
    label: v.name,
    value: itVendorNumericId(v.id),
    has_quoted: true,
    has_marks: false,
    is_passed: false,
    is_cleared: false,
  ...
  return itLineItems.map((item, idx) => ({
    ...
    clauses: itClauses.map((c) => ({
      // Unique PER PRODUCT. The same five clauses apply to every line, but a
      // mark is stored against clause_id + vendor_id — sharing one id across
      // products meant scoring the access point also scored the switch, the
      ...
      max_marks: c.maxMark,
      files: [],
      // The SAME unique id the clause carries above. Reading marks with the
      // raw c.id while writing them against the per-product id meant every
      // cell rendered blank while the counters and the "all vendors are
      // evaluated" badge — which read the group — said the work was done.
