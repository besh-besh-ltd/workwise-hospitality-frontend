...
import { demoSession, allSessionClauses, isRejection } from "./writeRoutes";
import { poRecordById, toListRow, poLines, vendorForId, docFileName } from "./poState";
import { markFor, scoredResponse, evaluationResult, vendorStanding } from "./techScoring";
import { vendorNumericId, lineCharges, CHARGE_PROFILE } from "./demoQuotes";
import { threadCompareView, sessionCompareView, itCompareView, itVendorNumericId } from "./quoteCompareView";
import { itRfq, itLineItems, itVendors, itClauses, IT_RFQ_ID, IT_MIN_SCORE, IT_CATEGORY } from "@/data/ihg/itRfq";
...
const poDetail = (po) => {
  ...
  if (decided) {
    const ok = !isRejection(decided.decision);
  ...
  return {
    id: Number(po.id),
    po_number: po.id,
    status: decided ? (isRejection(decided.decision) ? "rejected" : "approved") : "pending",
    status_label: decided
      ? (isRejection(decided.decision) ? "Rejected" : "Approved")
      : po.status,
    awaiting_me: !decided,
    ...
    workflow,
    docs: [{ type: "po", name: `PO_${po.id}.pdf`, url: `/api/po/${po.id}/document` }],
    poPdfUrl: `/api/po/${po.id}/document`,
    comparison: [],
...
/**
 * The same detail payload for a purchase order raised during the demo.
 *
 * These used to 404: the handler resolved ids against the five seeded POs
 * only, so the one PO the golden thread actually produces could not be opened
 * — and with it neither Initiate nor the document were reachable.
 */
const sessionPoDetail = (rec) => {
  const row = toListRow(rec);
  const lines = poLines(rec);
  const raiser = person("purchase");
  const approverP = person("finance");
  const isDraft = rec.status === "draft";
  const decided = rec.status === "approved" || rec.status === "rejected";

  const workflow = [
    { step: 1, status: "done", title: "PO created", by: raiser.name, when: rec.created_at, policy: null },
    {
      step: 2,
      // Reads "Drafted" until it is initiated — the distinction the whole
      // draft/issued story rests on.
      status: isDraft ? "current" : "done",
      title: isDraft ? "PO drafted" : "PO initiated",
      by: raiser.name,
      when: rec.initiated_at || rec.created_at,
      policy: null,
    },
    {
      step: 3,
      status: isDraft ? "pending" : decided ? (rec.status === "approved" ? "done" : "rejected") : "current",
      title: "Financial approval",
      by: decided ? approverP.name : null,
      when: rec.approved_at || null,
      policy: "Limit ₹25 L",
      level: 2,
      decision_rule: "ANY",
      approvers: [approver(approverP, decided ? (rec.status === "approved" ? "APPROVED" : "REJECTED") : "PENDING")],
      approver_summary: {
        total_active: 1,
        approved: rec.status === "approved" ? 1 : 0,
        rejected: rec.status === "rejected" ? 1 : 0,
        pending: decided ? 0 : 1,
        removed: 0,
      },
    },
  ...
  const keyDates = [{ k: "Drafted", v: rec.created_at }];
  if (rec.initiated_at) keyDates.push({ k: "Initiated", v: rec.initiated_at });
  if (rec.approved_at && rec.status === "approved") {
    keyDates.push({ k: "Approved", v: rec.approved_at });
    // Issuing to the vendor is the step after approval. It is shown as a dated
    // event rather than a `sent` status because the legacy PO card has no
    // `sent` key and would render it as "Draft".
    keyDates.push({ k: "Issued to vendor", v: rec.approved_at });
  }

  const activity = [
    { by: raiser.name, when: rec.created_at, text: `Draft purchase order raised from RFQ #${rec.rfq_no}` },
  ];
  rec.merged_from.forEach((m) => {
    activity.push({
      by: raiser.name,
      when: m.at,
      text: m.from_po_id
        ? `Combined draft #${m.from_po_id} into this purchase order`
        : "Another awarded line was added to this draft",
    });
  });
  if (rec.initiated_at) {
    activity.push({ by: raiser.name, when: rec.initiated_at, text: `Initiated as ${rec.po_number} · document generated` });
  }
  if (rec.approved_at) {
    activity.push({
      by: approverP.name,
      when: rec.approved_at,
      text: rec.status === "approved" ? "Approved and issued to the vendor" : "Rejected",
    });
  }

  return {
    id: Number(rec.id),
    po_number: rec.po_number || String(rec.id),
    status: rec.status,
    status_label: row.status_label,
    awaiting_me: rec.status === "pending_approval",
    covers_all_products: rec.covers_all_products !== false,
    current_step_label: row.current_step_label,
    current_approvers: rec.status === "pending_approval" ? [{ name: approverP.name }] : [],
    total_value: row.total_value,
    pricing: { total: row.total_value },
    vendor: {
      name: row.vendor_name,
      city: vendorForId(rec.finalized_vendor_id)?.city || "",
      gst: vendorForId(rec.finalized_vendor_id)?.gst || "",
      on_time_pct: vendorForId(rec.finalized_vendor_id)?.performance?.onTimePct || null,
    },
    rfq: { number: rec.rfq_no, id: Number(rec.rfq_no) },
    hotel: { name: propertiesById["ic-mumbai"]?.name },
    hotel_name: propertiesById["ic-mumbai"]?.name,
    items: lines.map((l) => ({
      name: l.name,
      quantity: l.qty,
      unit_price: l.rate,
    ...
    workflow,
    // No document until it is initiated — which is what leaves "Download PO"
    // disabled on a draft with "PO document not available yet".
    docs: rec.doc_url ? [{ type: "po", name: docFileName(rec.po_number), url: rec.doc_url }] : [],
    poPdfUrl: rec.doc_url || null,
    comparison: [],
    payment_terms: [
      { num: 1, name: "On delivery and GRN acceptance", due: "45 days from GRN", pct: 100, amount: row.total_value },
    ],
    tech_eval: [],
    key_dates: keyDates,
    activity,
    ...
    initiator: { name: raiser.name, email: raiser.email },
    project_details: rec.project_name ? { name: rec.project_name } : null,
  };
};
...
const techEvalGroups = (rfqId) => {
  ...
  // The linen thread carries no clauses on purpose: it is the purely
  // commercial story, its comparison screen says so, and giving it clauses
  // would lock its own negotiation stage behind an evaluation that was never
  // meant to exist. #535970 is the RFQ that demonstrates technical gating.
  const source = isSession ? written : [];
    ...
    return {
      ...
      clauses: rows.map((c) => ({
        ...
        vendor_responses: vendors.map((v) => {
          ...
          return scoredResponse(v, c.clause_id, vendorResponseText(slug, c, item), "2026-06-20T10:00:00+05:30");
        }),
      })),
    };
  });
...
/**
 * The IT RFQ's detail record. Carries clauses, so "Technical configured" reads
 * Yes and the evaluation stage is a real one.
 */
const itRfqById = () => ({
  ...rfqById(),
  id: Number(IT_RFQ_ID),
  ...
  hotel_ids: itRfq.propertyIds.map((p) => propertiesById[p]?.numericId),
  hotel_name: itRfq.propertyIds.map((p) => propertiesById[p]?.shortName).join(", "),
  department_name: itRfq.department,
  contact_name: person(itRfq.ownerId).name,
  contact_number: person(itRfq.ownerId).phone,
  bid_end_date: iso(itRfq.closesOn),
  is_quotes_present: true,
  // Drives the header control. Present only while the gate is still shut.
  demo_awaiting_technical: !demoSession.techComplete?.[IT_RFQ_ID],
  demo_awaiting_publish: false,
  demo_awaiting_close: false,
  products: itLineItems.map((item, idx) => {
    ...
    return {
      ...
      spec: rows,
      product_specs: rows,
      vendors_count: itVendors.length,
      participated_vendors_count: itVendors.length,
    };
  }),
});

/** The IT RFQ's clauses, grouped per product with every vendor to score. */
const itTechEvalGroups = () =>
  itLineItems.map((item, idx) => {
    const clauses = itClauses.map((c) => ({
      clause_id: c.id * 10 + (idx + 1),
      weightage: c.maxMark,
    }));

    // Per product: a vendor can clear the bar on one line and miss it on
    // another, so the standing is computed against THIS product's clauses.
    const vendors = itVendors.map((v, vIdx) => {
      const id = itVendorNumericId(v.id);
      const standing = vendorStanding(clauses, id, IT_MIN_SCORE);
      return {
        vendor_id: id,
        rfq_product_vendor_id: 950 + vIdx,
        vendor_name: v.name,
        label: v.name,
        value: id,
        has_quoted: true,
        ...
        is_replaced_out: false,
        evaluation_round: 1,
        evaluated_by: standing.has_marks ? "Vivek Jaiswal" : null,
        approved_by: null,
        rank: vIdx + 1,
        ...standing,
      };
    ...
    return {
      ...
      product_name: item.name,
      minimum_passing_score: IT_MIN_SCORE,
      vendors,
      clauses: itClauses.map((c, cIdx) => ({
        // Unique PER PRODUCT — the same five clauses apply to every line, and
        // a mark is stored against clause_id + vendor_id. One shared id meant
        // scoring the access point also scored the switch and the UPS.
        clause_id: clauses[cIdx].clause_id,
        rfq_product_id: idx + 1,
        clause_text: c.text,
        clause_type: "clause",
        weightage: c.maxMark,
        max_marks: c.maxMark,
        files: [],
        vendor_responses: vendors.map((v) => scoredResponse(
          v,
          clauses[cIdx].clause_id,
          `${v.vendor_name} confirms compliance with ${c.title.toLowerCase()} for ${item.name}. Datasheet and certificate attached.`,
...
/**
 * The negotiation rounds run on an RFQ.
 *
 * Two completed rounds on the thread: an opening pass across the towelling
 * lines, then a narrower one on freight and packaging only. Shared by the
 * per-product cards and the RFQ-wide summary so a round cannot appear on one
 * and not the other.
 */
const negotiationRounds = (rfqId) => [
  {
    id: 9101,
    round_number: 1,
    rfq_id: rfqId,
    rfq_product_id: null,
    products: [1, 2, 3, 4, 5, 6].map((id) => ({ rfq_product_id: id })),
    status: "COMPLETED",
    created_by_name: person(rfq.ownerId).name,
    created_at: iso("2026-07-02"),
    end_date: iso("2026-07-09"),
    vendors_invited: 6,
    vendors_responded: 5,
    savings_delivered: 1180000,
    note: "Opening round across the towelling lines. Five of six suppliers improved.",
  },
  {
    id: 9102,
    round_number: 2,
    rfq_id: rfqId,
    rfq_product_id: null,
    products: [1, 2, 3, 4].map((id) => ({ rfq_product_id: id })),
    status: "COMPLETED",
    created_by_name: person(rfq.ownerId).name,
    created_at: iso("2026-07-21"),
    end_date: iso("2026-07-28"),
    vendors_invited: 4,
    vendors_responded: 4,
    savings_delivered: 640000,
    note: "Second pass on freight and packaging only; base rates left untouched.",
  },
];


/**
 * Whether the persona currently signed in may approve a finalised product.
 *
 * The adapter runs in the browser, so the seeded session is readable. Falls
 * back to true on the server (SSR has no session and renders nothing that
 * depends on this).
 */
const currentPersonaCanApprove = () => {
  if (typeof window === "undefined") return false;
  try {
    const email = window.localStorage.getItem("current-user-email") || "";
    const person = Object.values(peopleById).find((p) => p.email === email);
    return !!person?.can?.approvePO;
  } catch (_) {
    return false;
  }
};
...
const rfqById = () => ({
  ...
  is_quotes_present: true,
  po_completed: false,
  unseen_query_count: 2,
  // Everything below was empty, which left a reader with no idea what this RFQ
  // actually asks of a supplier — the terms card, the clarification window and
  // the attached T&C were all blank.
  vendor_clarification_date: iso("2026-06-18"),
  clarification_start_date: iso("2026-06-05"),
  comment:
    "Rates to hold for the full financial year. Deliveries are to the five properties named above, " +
    "against a call-off schedule issued quarterly. Part-shipments are accepted only with prior written " +
    "agreement from the sourcing desk.",
  close_comment: null,
  copies: 0,
  copied_from: null,
  terms: [
    { id: 1, term_id: 1, name: "Payment", term_content: "Payment 30 days from GRN, against a valid tax invoice and a signed delivery challan." },
    { id: 2, term_id: 2, name: "Delivery", term_content: "Delivered to store at each property. Freight, insurance and unloading are to the supplier's account unless quoted separately and accepted." },
    { id: 3, term_id: 3, name: "Quality", term_content: "Goods to match the approved sample and the stated GSM/thread count within \u00b13%. IHG may draw a sample from any lot at GRN." },
    { id: 4, term_id: 4, name: "Rejection", term_content: "Rejected material to be lifted within 14 days at the supplier's cost. Replacement within the original lead time." },
    { id: 5, term_id: 5, name: "Rate validity", term_content: "Quoted rates firm for the contract term. Any revision requires 30 days' notice and IHG's written acceptance." },
    { id: 6, term_id: 6, name: "Compliance", term_content: "Valid GSTIN, OEKO-TEX Standard 100 and a current MSME/Udyam registration where claimed. Certificates to remain valid through the term." },
  ],
  TERM_files: [
    { name: "IHG_Standard_Purchase_Terms_FY2026-27.pdf", url: "#", size: "184 KB" },
  ],
...
const detailRoutes = [
  /* purchase order */
  {
    ...
    handler: ({ params }) => {
      ...
      if (po) return poDetail(po);
      const rec = poRecordById(demoSession, params.id);
      return rec ? sessionPoDetail(rec) : undefined;
    },
  },
  {
    /**
     * Legacy PO detail, behind /dashboard/buyer/purchase-order.
     *
     * Deliberately NOT raw(): getPoDetails returns res.data, so this one goes
     * through the wrapper. Registered after listRoutes so /po/list, /po/awaiting,
     * /po/tracking, /po/analytics and the exports match first — they share this
     * shape.
     */
    method: "get",
    path: "/po/:poId",
    handler: ({ params }) => {
      ...
      if (po) return poDetail(po);
      const rec = poRecordById(demoSession, params.poId);
      return rec ? sessionPoDetail(rec) : undefined;
    },
  },
  {
  ...
  },
  {
    ...
    handler: ({ params }) => {
      ...
      if (id === IT_RFQ_ID) return itRfqById();
      return demoSession.publishedRfqs?.[id] ? sessionRfqById(id) : rfqById();
    },
  },
  { method: "get", path: "/rfq/:id/lineage", handler: () => ({ copied_from: null, copies: [] }) },
  {
    ...
    handler: ({ params }) => {
      ...
      const id = String(params.id);
      if (id === IT_RFQ_ID) return raw({ data: itTechEvalGroups() });
      return raw({ data: techEvalGroups(id) });
    },
  },
  {
    method: "get",
    path: "/negotiation/rounds/:id",
    // ViewRFQ reads this as `res.data` and then does Array.isArray() on it, so
    // the payload has to BE the array. Returning { rounds: [...] } counts as
    // zero rounds and every product reads "No negotiation rounds".
    //
    // The product id arrives in the query string rather than as an axios param,
    // so it is read off the URL.
    handler: ({ params, config }) => {
      const pid = Number((String(config?.url || "").match(/rfq_product_id=(\d+)/) || [])[1]);
      const rounds = negotiationRounds(Number(params.id));
      if (!pid) return rounds;
      return rounds.filter((r) => r.products.some((p) => p.rfq_product_id === pid));
    },
  },
  {
    method: "get",
    ...
    handler: ({ params }) => {
      ...
      // Counted from the SAME payload /rfq/get-clauses serves. Counting from
      // the raw clause fixture instead is what let the rail claim "5 clauses
      // configured" while every product underneath read "No Clauses Available"
      // and the header said Technical configured: No — three answers to one
      // question, from three different sources.
      const groups = id === IT_RFQ_ID ? itTechEvalGroups() : techEvalGroups(id);
      // Per product, not the grid total. The same five clauses apply to each
      // line, so summing gives 25 — which reads as twenty-five distinct
      // clauses rather than five applied five times.
      const perProduct = groups.map((g) => g.clauses.length);
      const uniform = perProduct.length > 0 && perProduct.every((n) => n === perProduct[0]);
      const clauseCount = uniform ? perProduct[0] : perProduct.reduce((a, b) => a + b, 0);
      const hasClauses = clauseCount > 0;
      const technicalSignedOff = !!demoSession.techComplete?.[id];

      // What has been awarded on this RFQ, and how far each one has got.
      const awardKeys = Object.keys(demoSession.awards || {}).filter((k) => k.startsWith(`${id}:`));
      const awardsApproved = awardKeys.filter((k) => demoSession.quoteApprovals?.[k] === "approved").length;
      const awardsPending = awardKeys.filter((k) => !demoSession.quoteApprovals?.[k]).length;
        ...
        stages: [
          ...
          },
          {
            ...
            state: !hasClauses
              ...
              : technicalSignedOff
                ? "complete"
                : published ? "active" : "locked",
            summary: !hasClauses
              ? undefined
              : technicalSignedOff
                ? `${clauseCount} clauses scored · vendors cleared`
                : `${clauseCount} clause${clauseCount === 1 ? "" : "s"} configured`,
          },
          {
            key: "negotiation-award",
            label: "Negotiation & award",
            // Locked behind the technical evaluation whenever clauses exist —
            // negotiating on price before the technical verdict is exactly what
            // the evaluation is meant to prevent. With no clauses there is
            // nothing to wait for, so quotes alone unlock it.
            state: !published
              ? "locked"
              : hasClauses && !technicalSignedOff
                ? "locked"
                : "active",
            summary: !published
              ? undefined
              : hasClauses && !technicalSignedOff
                ? "Locked until the technical evaluation is complete"
                : "Quotes in — ready to negotiate or award",
          },
          {
            key: "purchase-order",
            label: "Purchase order",
            // Finalising does not raise a PO — the approver has to sign off
            // first. The stage stays locked until then, but it now says what
            // it is waiting for instead of a bare "Not reached", which read
            // as though finalising had failed.
            state: awardsApproved > 0 ? "active" : "locked",
            summary: awardsApproved > 0
              ? `${awardsApproved} product${awardsApproved === 1 ? "" : "s"} approved — PO draft raised`
              : awardsPending > 0
                ? `Waiting on approval of ${awardsPending} finalised product${awardsPending === 1 ? "" : "s"}`
                : undefined,
          },
        ],
  ...
  },
  {
    ...
    // The four counters above the matrix. Computed from the marks actually
    // given, so they move as the evaluator scores instead of sitting at zero.
    handler: ({ params }) => {
      const id = String(params.id);
      const groups = id === IT_RFQ_ID ? itTechEvalGroups() : techEvalGroups(id);
      if (!groups.length) {
        return { completed_product_ids: [], products_completed: 0, products_in_progress: 0, vendors_passed: 0, vendors_failed: 0 };
      }
      ...
      groups.forEach((g) => {
        const rows = evaluationResult([g], g.minimum_passing_score);
        const anyScored = rows.some((r) => r.scored_clauses > 0);
        const allDone = rows.every((r) => r.is_evaluated);
        if (allDone) completed.push(g.rfq_product_id);
        else if (anyScored) inProgress += 1;
        passed += rows.filter((r) => r.is_passed).length;
        failed += rows.filter((r) => r.is_failed).length;
      });

      return {
        completed_product_ids: completed,
        products_completed: completed.length,
        products_in_progress: inProgress,
  ...
  },
  {
    ...
    // Keyed by rfq_product_id. useTechEvalWorkflow derives the whole editable
    // state from this: with no `vendors` block it lands on AWAITING_NEXT_ROUND,
    // which locks every score chip — the evaluation renders but cannot be used.
    handler: ({ params }) => {
      const pid = Number(params.id);
      const group = itTechEvalGroups().find((g) => g.rfq_product_id === pid);
      if (!group) {
        return { is_complete: false, rounds: [], vendors: { selected: [], pending_evaluation: [], passed_verified: [], failed_verified: [] } };
      }

      const rows = evaluationResult([group], group.minimum_passing_score);
      const asVendor = (r) => ({ vendor_id: r.vendor_id, vendor_name: r.vendor_name, score: r.score, percentage: r.percentage });

      return {
        is_complete: false,
        current_round: 1,
        required_passed_vendors: 1,
        rounds: [],
        vendors: {
          selected: rows.map(asVendor),
          pending_evaluation: rows.filter((r) => !r.is_evaluated).map(asVendor),
          passed_verified: rows.filter((r) => r.is_passed).map(asVendor),
          failed_verified: rows.filter((r) => r.is_failed).map(asVendor),
        },
      };
    },
  },
  {
    method: "post",
    path: "/rfq/get-tech-evaluation-result",
    // Pass/fail per vendor, rolled up from the marks actually given. Derived
    // from the same grouped payload the matrix renders, so the verdict and the
    // cells can never disagree.
    handler: ({ body }) => {
      const id = String(body?.rfq_id || "");
      const groups = id === IT_RFQ_ID ? itTechEvalGroups() : techEvalGroups(id);
      const minScore = id === IT_RFQ_ID ? IT_MIN_SCORE : MIN_SCORE;
      const rows = evaluationResult(groups, minScore);
      const done = rows.length > 0 && rows.every((r) => r.is_evaluated);
      return raw({ status: done ? 1 : 0, data: rows, minimum_passing_score: minScore });
    },
  },
  {
    method: "get",
  ...
  },
  {
    ...
    handler: ({ params, config }) => {
      ...
      const technicalDone = !!demoSession.techComplete?.[id];
      const view = id === IT_RFQ_ID
        ? itCompareView(id, withFreight, technicalDone)
        : rec && collected
      ...
      // Who is looking. The adapter runs in the browser, so the signed-in
      // persona is readable — and it decides whether a finalised product is
      // waiting on THIS user. Without awaiting_me the screen never enters
      // approver mode, so a finalised RFQ sits on "Awaiting approval" with
      // every footer control correctly disabled and no way forward.
      const canApprove = currentPersonaCanApprove();

      view.products.forEach((prod) => {
        const award = demoSession.awards?.[`${id}:${prod.id}`];
        if (!award) return;
        const decision = demoSession.quoteApprovals?.[`${id}:${prod.id}`];
        prod.finalized_vendor = award.vendor_id;
        prod.finalized_by = peopleById.purchase?.name || null;

        if (decision === "approved") {
          prod.state = "approved";
          prod.awaiting_me = false;
        } else if (decision === "rejected") {
          prod.state = "rejected";
          prod.awaiting_me = false;
          prod.reject_info = { reason: "Sent back by the approver.", by: peopleById.finance?.name };
        } else {
          prod.state = "pending";
          prod.awaiting_me = canApprove;
        }
      });
  ...
  },
  {
    ...
    // Was an empty list, so every product read "No negotiation rounds" and the
    // commercial half of the page looked like nothing had happened on an RFQ
    // whose whole story is a negotiation.
    handler: ({ params }) => {
      const rounds = negotiationRounds(Number(params.id));
      return {
        rounds,
