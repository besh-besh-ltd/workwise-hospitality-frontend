  ...
  return itLineItems.map((item, idx) => ({
    ...
    clauses: itClauses.map((c) => ({
      ...
      vendor_responses: vendors.map((v) => ({
        vendor_id: v.vendor_id,
        rfq_product_vendor_id: v.rfq_product_vendor_id,
        vendor_response: `${v.vendor_name} confirms compliance with ${c.title.toLowerCase()} for ${item.name}. Datasheet and certificate attached.`,
        response_timestamp: "2026-09-20T10:00:00+05:30",
        score_timestamp: "2026-09-20T10:00:00+05:30",
        buyer_marks: null,
        buyer_remark: null,
        scorer_name: null,
        files: [],
