...
import { activePos, toListRow, toLegacyRow, vendorNameForId } from "./poState";
 ...
 */
const sessionPoRows = () => activePos(demoSession).map(toListRow);
...
const poRow = (p) => {
  ...
  return {
    ...
    status: demoSession.poDecisions[p.id]
      ? (isRejection(demoSession.poDecisions[p.id].decision) ? "rejected" : "approved")
      : "pending",
...
const listRoutes = [
  ...
  /* Purchase orders */
  {
    ...
    handler: ({ query }) => {
      ...
      // The tabs read `status_counts.action_required` and send their key back
      // as `status`. Both were ignored — every tab returned the whole list and
      // "Pending for me" always read 0. With drafts in the register now, that
      // would be actively misleading rather than merely wrong.
      const isPending = (r) => r.status === "pending" || r.status === "pending_approval";
      const buckets = {
        all: () => true,
        "action-required": isPending,
        approved: (r) => r.status === "approved",
        rejected: (r) => r.status === "rejected",
      };
      const tab = String(query.status || "all");
      const shown = rows.filter(buckets[tab] || buckets.all);
      return raw({
        data: shown,
        total_items: shown.length,
        page: Number(query.page) || 1,
        limit: Number(query.limit) || 20,
        status_counts: {
          all: rows.length,
          action_required: rows.filter(isPending).length,
  ...
  {
    /**
     * The RFQ-scoped PO listing behind /dashboard/buyer/purchase-order.
     *
     * This is the only screen in the product that carries "Combine N into one",
     * so it has to work for merge to be reachable at all. `getPoData` returns
     * the whole response, so this one is raw() and the caller reads
     * value.data / value.total / value.approval_level.
     */
    method: "get",
    path: "/po/rfq/:rfqId",
    handler: ({ params }) => {
      const canApprove = !!persona().can.approvePO;
      const rows = activePos(demoSession)
        .filter((po) => String(po.rfq_no) === String(params.rfqId))
        .map((po) => toLegacyRow(po, canApprove));
      return raw({ data: rows, total: rows.length, approval_level: canApprove ? 2 : 1 });
    },
  },
  // The three Excel exports are blob: GETs. They were unfixtured, so they fell
  // to the catch-all and saveBlob() called createObjectURL on a plain object.
  // They also sit at the same shape as /po/:po_id, so they must be matched
  // before it.
  ...["/po/export", "/po/tracking/export", "/po/analytics/export"].map((path) => ({
    method: "get",
    path,
    handler: () => raw(new Blob(["Export is not available in the demo."], { type: "text/plain" })),
  })),
  {
    method: "get",
    path: "/po/awaiting",
    // A DRAFT is waiting on nobody — it has not been initiated yet. Splicing
    // every session PO in here regardless put un-initiated drafts under
    // "Awaiting your approval" and inflated the approver's queue.
    handler: () => raw({
      data: persona().can.approvePO
        ? [...sessionPoRows().filter((p) => p.status === "pending_approval"), ...stillPending().map(poRow)]
        : [],
    }),
  },
  {
    ...
    handler: () => {
      ...
      return raw({
        activeCount: purchaseOrders.length + sessionPoRows().length,
        // Counts the same set the /po/awaiting list returns, which it did not
        // before: the queue showed session POs the counter never knew about.
        awaitingYou: mine.length + sessionPoRows().filter((p) => p.status === "pending_approval").length,
        awaitingOldestDays: Math.max(0, ...mine.map((p) => p.pendingDays)),
        inTransit: 0,
        vendorAccepted: 0,
        vendorAcceptancePending: 0,
        avgDeliveryDays: 28,
        approvedThisMonth: Object.values(demoSession.poDecisions).filter((d) => !isRejection(d.decision)).length,
        approvedDeltaPct: 0,
